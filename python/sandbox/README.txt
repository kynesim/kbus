...
pointers (which would by definition by NULL for an "entire" message).
...
<binder_id> and <name_len> fields are converted to/from network order.

All other message data is transmitted as given (i.e., as a byte stream).
