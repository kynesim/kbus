...
namespace cppkbus
{
    ...
    namespace Error
    {
        typedef enum
        {
            ...
            // Following are the errno.h values that KBUS uses for its own
            // purposes
    ...
    /*
     * A KBUS message uses various flags to indicate how it should behave.
     */
    namespace MessageFlags
    {
        //! This message requires a reply. This flag is what tells a message
        // that it is a Request.
        static const uint32_t WantReply = (1<<0);

        //! This message was received by you and you are to reply. This flag is
        // set by KBUS when a message is receieved.
        static const uint32_t WantYouToReply = (1<<1);

        //! This is a synthetic message, internally generated by KBUS itself
        // (normally to reply when the intended recipient cannot)
        static const uint32_t Synthetic = (1<<2);

        //! This is an urgent message. Setting this flag on a message requests
        // that it be delivered to the front of the recipients message queue,
        // instead of (as normal) the back.
        static const uint32_t Urgent = (1<<3);

        //! All or wait. This requests that KBUS send this message to all
        // recipients before returning
        static const uint32_t AllOrWait = (1<<8);

        //! All or fail. This requests that KBUS fail if this message could not
        // immediately be sent to all recipients
    ...
    class Ksock
    {
        public:
            ...
            /*
             * Return a file descriptor (for this Ksock's Device) that you can poll
             *
             * On the other hand, it can be simpler just to use WaitForMessage,
             * if that will do all you want.
             */
