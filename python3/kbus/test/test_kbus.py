...
def check_IOError(expected_errno, fn, *stuff):
    """When calling fn(*stuff), check for IOError with the given errno.
...
class TestKernelModule:
    ...
    def test_rw_single_file(self):
        ...
        try:
            ...
            name1 = '$.Fred.Jim'
            data1 = array.array('L', b'datadata').tostring()

            name2 = '$.Fred.Bob.William'
            data2 = array.array('L', b'This is surely some data').tostring()
    ...
    def test_read_write_2files(self):
        ...
        try:
            ...
            try:
                ...
                assert f1.read_data(1) == b''

                # No more messages on f2
                assert f2.next_msg() == 0
                assert f2.read_data(1) == b''
            finally:
                assert f2.close() is None
        finally:
            assert f1.close() is None
    ...
    def test_reads(self):
        ...
        with RecordingKsock(0, 'rw', self.bindings) as f0:
            ...
            with RecordingKsock(0, 'r', self.bindings) as f1:
                ...
                data += f1.read_data(1)
                # The underlying file system is unbuffered under
                # Python3, so we no longer get anomalous lengths after
                # reading a little.
                assert f1.len_left() == length-1
        ...
        with RecordingKsock(0, 'rw', self.bindings) as f0:
            ...
            with RecordingKsock(0, 'r', self.bindings) as f1:
                ...
                def test(f0, f1, m):
                    ...
                    data = m.to_bytes()
                    for ch in data:
                        f0.write_data(bytes((ch,)))        # which also flushes
                        assert f1.next_msg() == 0
                    f0.send()
                    r = f1.read_next_msg()
                    assert r.equivalent(m)
    ...
    def test_cant_write_while_sending(self):
        ...
        with Ksock(0, 'rw') as sender:
            with Ksock(0, 'rw') as listener:
                ...
                check_IOError(errno.EALREADY, listener.write_data, b'fred')
    ...
    def test_too_many_unsent_unbind_events(self):
        ...
        with Ksock(0, 'rw') as first:
            ...
            with Ksock(0, 'rw') as second:
                second_id = second.ksock_id()
                for ii in range(TOO_MANY_MESSAGES):
